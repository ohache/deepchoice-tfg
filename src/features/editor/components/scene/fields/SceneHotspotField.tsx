...
import type { ID, Hotspot, HotspotActionType, Interaction, Effect } from "@/domain/types";
...
interface SceneHotspotFieldProps {
  ...
  onChangeAction: (hotspotId: ID, actionType: HotspotActionType) => void;
...
const ACTION_LABEL: Record<Exclude<HotspotActionType, "">, string> = {
  goToNode: "Ir a otra escena",
  addItem: "Coger item",
  removeItem: "Quitar item",
  setFlag: "Cambiar flag",
  startDialogue: "Hablar con PNJ",
  giveItemToNpc: "Dar item a PNJ",
  showText: "Mostrar texto",
};

const ACTION_TYPES: Array<Exclude<HotspotActionType, "">> = [
  "goToNode",
  "addItem",
  "removeItem",
  "setFlag",
...
function getPrimaryInteraction(hs: Hotspot): Interaction | undefined {
...
export function SceneHotspotField({ label = "Hotspots", active, onToggle, hotspots, canBindTargets, availableNodesByHotspotId,
  onChangeAction, onChangeTarget, onStartDrawing, activeDrawingHotspotId, onRemoveHotspot, onAddHotspot, hotspotErrors,
  noScenesMessage, hotspotsWithoutScenesMessage, emptyHotspotsMessage, resolveNodeLabel, selectPlaceholderWhenActive = "Selecciona destino…",
  selectPlaceholderWhenDisabled = "No hay escenas disponibles", hasImage, focusedHotspotId, onFocusHotspot, onClearFocus }: SceneHotspotFieldProps) {
    const hasAnyGoToNode = hotspots.some((hs) => {
      const pe = getPrimaryEffect(hs);
      return isGoToNodeEffect(pe);
    });
    ...
    const lastActionType = (lastPrimaryEffect?.type ?? "") as HotspotActionType;
  
    const lastGo = lastHotspot && isGoToNodeEffect(lastPrimaryEffect) ? lastPrimaryEffect : undefined;
    const lastTargetOk = !!lastGo?.targetNodeId;
    const lastShapeOk = !!lastHotspot?.shape;
  
    const isLastValid = !lastHotspot ? true : lastActionType === "goToNode"
          ? (hasImage && (!canBindTargets || lastTargetOk) && lastShapeOk) : true;
  
    const canAddHotspot = isLastValid;
  
    const addDisabledReason = !isLastValid ? "Completa el hotspot anterior (acción y zona dibujada) antes de añadir otro." : "";
            ...
            {!canBindTargets && (
              <div className="rounded-md border border-slate-800 bg-slate-950/40 px-3 py-2">
                <p className="text-[11px] text-slate-400">
                  {hotspots.length === 0
                    ? noScenesMessage
                    : (hotspotsWithoutScenesMessage ?? noScenesMessage)}
                </p>
            ...
            {showImageRequiredWarning && (
              <div className="rounded-md border border-amber-700/40 bg-amber-900/10 px-3 py-2">
                <p className="text-[11px] text-amber-200">
                  Para usar hotspots debes <span className="font-semibold">cargar una imagen</span>{" "}
                  (es necesaria para dibujarlos).
                </p>
              </div>
            )}
            ...
            {hotspots.length > 0 && (
              <div className="space-y-2" onClick={(e) => e.stopPropagation()}>
                {hotspots.map((hs, index) => {
                  ...
                  const firstActionType = (primaryEffect?.type ?? "") as HotspotActionType;
                  ...
                  const hasCurrentInAvailable =
                    !!currentTargetId && availableNodes.some((n) => n.id === currentTargetId);
  
                  const resolvedLabel =
                    (currentTargetId && resolveNodeLabel?.(currentTargetId)) ?? currentTargetId;
                      ...
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <div className="flex items-center gap-2">
                          <button
                            ...
                            title={
                              canEditZone
                                ? (isDrawingThis ? "Dibujando…" : "Editar zona (redibujar)")
                                : disabledReason
                            }
                          >
                      ...
                      {!isExpanded ? (
                        <div className="mt-2 text-[11px] text-slate-400">
                          <div className="flex items-center justify-between gap-2">
                            <span>
                              {firstActionType
                                ? `Acción: ${ACTION_LABEL[firstActionType as Exclude<HotspotActionType, "">]}`
                                : "Acción: (sin seleccionar)"}
                            </span>
                          ...
                          <div className="space-y-1">
                            ...
                            <select
                              ...
                              onChange={(e: React.ChangeEvent<HTMLSelectElement>) =>
                                onChangeAction(hs.id, e.target.value as HotspotActionType)
                              }
