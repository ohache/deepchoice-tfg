...
import type { MusicTrackDef } from "@/domain/types";
...
export function HistoryMusicPanel() {
  ...
  const musicTrackList = project?.musicTracks ?? [];
  const selectedMusicTrack = selectedTrackId && project ? musicTrackList.find((s) => s.id === selectedTrackId) ?? null : null;
  ...
  const audio = useAudioFileDraft({
    ...
    messages: {
      duplicateFieldError: "Ya existe una pista que usa este archivo de audio.",
      duplicateToastTitle: "Archivo duplicado",
      duplicateToastBody: "Ya hay una pista usando ese archivo.",
  ...
  const panel = useAssetDraftPanel<MusicTrackDef>({
    ...
    items: musicTrackList,
    setSelectedId: setSelectedTrackId,

    onLoadDraftFieldsFromSelected: (track) => {
      setDraftName(track.name);
      setFieldErrors({});
      audio.resetAudioDraft();

      const assetPath = (project?.assets ?? []).find((a) => a.kind === "music" && a.id === track.id)?.file?.trim() ?? "";
  ...
  const modeTitle = (mode: "none" | "new" | "edit") => mode === "new" ? "Nueva pista" : mode === "edit" ? "Editar pista" : "Detalle de pista";
  ...
  const validateDraft = (): boolean => {
    ...
    const { ok, errors } = validateMusicDraft(
      ...
      { mode: mode === "edit" ? "edit" : "new", project, currentTrackId: selectedTrackId ?? undefined }
    );
  ...
  const handleSave = () => {
    ...
    if (mode === "new") {
      ...
      toast.success("Música creada", `“${displayName}”`);
      panel.reset();
      return;
    }
      ...
      <div className="flex gap-4 h-full">
        ...
        <aside className="w-1/3 rounded-lg bg-slate-950 flex flex-col overflow-hidden">
          <button
            ...
            className="px-3 py-2 text-base font-semibold bg-cyan-800 hover:bg-cyan-700 text-white rounded-t-lg"
          >
            + Añadir pista
          </button>

          <div className="flex-1 overflow-y-auto text-[15px]">
            {musicTrackList.length === 0 ? (
              <p className="p-4 text-xs text-slate-320 text-center">No hay pistas de música en el proyecto</p>
            ) : (
                    ...
                    <li key={track.id}>
                      <button
                        ...
                        className={"w-full text-left px-6 py-3 text-[15px] border-t border-t-black " +
                          (isSelected ? "bg-cyan-900/60 text-slate-50" : "hover:bg-cyan-900/60 text-slate-200")}
                      >
                        <span className="block w-full overflow-hidden text-ellipsis whitespace-nowrap">{track.name}</span>
                      </button>
                    </li>
                  );
                })}
          ...
          <div className="p-4 flex-1 flex flex-col">
            {mode === "none" ? (
              <p className="text-[11px] text-slate-200 text-center">
                Selecciona una pista en la lista de la izquierda o pulsa <span className="font-semibold">“Añadir pista”</span>{" "}
              ...
              <>
                ...
                <div className="mb-2 mt-2">
                  <label className="block text-[13px] text-slate-200 mb-1 text-center">Archivo de música</label>
                  ...
                  <div
                    className={"group relative mt-1.5 px-3 py-3.5 rounded-md flex flex-col items-center justify-center text-[12px] " +
                      ...
                      (audio.isDragging
                        ...
                        : "border-cyan-800 bg-slate-900/40 " + (audio.isHoveringSelectButton ? "" : "hover:bg-cyan-900/60"))
                    }
                ...
                <audio ref={audio.audioRef} src={audio.previewUrl ?? undefined} loop={true} className="hidden" />
                ...
                <div className="flex justify-center mt-2 mb-6">
                  <button
                    ...
                    className={"min-w-35 px-6 py-2 rounded-md text-[12px] font-semibold text-white " +
                      ...
                      (audio.isPlaying ? "bg-red-800 hover:bg-red-700" : "bg-cyan-700 hover:bg-cyan-600")}
                  >
                ...
                <div className="mt-auto flex justify-between">
                  ...
                  <div className="flex gap-3 panel--music">
                    <button
                      type="button"
                      onClick={panel.reset}
