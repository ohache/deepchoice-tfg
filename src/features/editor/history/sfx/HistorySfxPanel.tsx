...
export function HistorySfxPanel() {
  ...
  const loadDraftFromSelectedSfx = (sfx: SoundEffectDef) => {
    ...
    const assetPath = (project?.assets ?? []).find((asset) => asset.kind === "sfx" && asset.id === sfx.id)?.file?.trim() ?? "";
  ...
  const panel = useAssetDraftPanel<SoundEffectDef>({
    ...
    onLoadDraftFieldsFromSelected: loadDraftFromSelectedSfx,
    ...
    <div className="max-w-[900px] mx-auto rounded-xl border-3 border-slate-700 bg-slate-900 p-4 space-y-3">
      <div className="flex gap-4 h-full">
        ...
        <section className="relative flex-1 rounded-lg border border-indigo-700 bg-slate-950 text-sm text-slate-100 flex flex-col overflow-hidden">
          {mode !== "none" && (
            <img
              src={publicPath("ui/sfx-watermark.png")}
          ...
          <div className="p-4 flex-1 flex flex-col">
            {mode === "none" ? (
              <p className="text-[11px] text-slate-200 text-center">
                Selecciona un efecto en la lista de la izquierda o pulsa{" "}
                <span className="font-semibold">“Añadir efecto”</span> para crear uno nuevo
              </p>
            ) : (
              <>
                <div className="mb-2">
