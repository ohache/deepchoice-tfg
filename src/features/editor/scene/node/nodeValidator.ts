...
import { nodeSchema, nodeDraftSchema } from "@/features/editor/scene/node/nodeSchemas";
...
const duplicateLayerIdError = "No puede haber dos capas con la misma imagen.";
...
function applyBusinessRules(nodeLike: NodeLikeForBusinessRules, errors: NodeFieldErrors): void {
  ...
  const hasBase = layers.some((l) => l.when == null);
