...
export function InventoryOverlay({ open, items, onClose, onSelectItem, onUseItemOnInventoryItem }: InventoryOverlayProps) {
  ...
  const slots = useMemo(() => { return Array.from({ length: INVENTORY_SLOTS }, (_, index) => items[index] ?? null); }, [items]);
  ...
  return (
    ...
    >
      ...
      <div
        ...
        className={`relative h-[600px] w-[900px] max-w-[94vw] max-h-[88vh] overflow-hidden rounded-2xl border border-slate-950/60 bg-cyan-950/70 shadow-2xl ${open
      ...
      >
        <div className="h-full p-5">
          <div className="grid h-full grid-cols-6 grid-rows-4 gap-4">
            {slots.map((item, index) => (
              <div
                key={item ? item.instanceId : `empty-${index}`}
                className="flex items-center justify-center rounded-lg border border-slate-700 bg-slate-950/55 p-3 shadow-inner"
              >
                {item ? (
                  <button
                    type="button"
                    ...
                    className={`flex h-full w-full items-center justify-center rounded-md ${pressedItem?.instanceId === item.instanceId
      ...
      {cursorPos.visible && (
        <img
          ...
          aria-hidden="true"
          draggable={false}
          className={`pointer-events-none fixed z-60 object-contain select-none ${isDraggingItem ? "" : "h-16 w-16"
            }`}
          style={
            isDraggingItem
              ? {
