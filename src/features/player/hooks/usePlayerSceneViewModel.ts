...
export function usePlayerSceneViewModel(project: Project | null, displayedNodeId: ID | null, displayedGameState: GameState | null,
  ...
  const activeText = useMemo(() => {
    ...
    const dockText = displayedGameState.gameEnded
      ? displayedGameState.ending?.dockText?.trim()
      : "";

    return dockText
      ? { ...baseText, text: dockText }
      : baseText;
  ...
  const hotspotsForStage = useMemo(() => {
    return (activeLayer?.hotspots ?? []).map((hotspot) => ({ hotspot, runtime: nodeRt?.hotspots?.[hotspot.id] ?? hotspot.initialState }));
  }, [activeLayer, nodeRt]);

  const placedItemsForStage = useMemo(() => {
    return (activeLayer?.placedItems ?? []).map((placedItem) => ({ placedItem, runtime: nodeRt?.placedItems?.[placedItem.id] ?? placedItem.initialState }));
  }, [activeLayer, nodeRt]);

  const placedPlayersForStage = useMemo(() => {
    return (activeLayer?.placedPlayers ?? []).map((placedPlayer) => ({ placedPlayer, runtime: nodeRt?.placedPlayers?.[placedPlayer.playerId] ?? placedPlayer.initialState }));
  }, [activeLayer, nodeRt]);

  const placedNpcsForStage = useMemo(() => {
    return (activeLayer?.placedNpcs ?? []).map((placedNpc) => ({ placedNpc, runtime: nodeRt?.placedNpcs?.[placedNpc.npcId] ?? placedNpc.initialState }));
  }, [activeLayer, nodeRt]);

  return { currentNode, activeLayer, activeText, activeImageSrc, hotspotsForStage, placedItemsForStage, placedPlayersForStage, placedNpcsForStage };
}