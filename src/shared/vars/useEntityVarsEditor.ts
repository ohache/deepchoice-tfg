...
type ValidateVarRowsResult = { ok: true; variables: VarDef[] } | { ok: false; errorsById: Record<string, VarRowErrors> };
...
type UseEntityVarsEditorResult = {
  ...
  validateVarRows: () => ValidateVarRowsResult;
  persistValidatedVars: (variables: VarDef[]) => void;
};
...
export function useEntityVarsEditor({ initialVars, createId = () => generateId.var(), onPersistRemove, onPersistSave, useDirtyTracking = false,
  ...
  const computeRowErrors = useCallback((row: VarRow) => {
  ...
  const updateVarRow = useCallback((id: string, patch: Partial<VarRow>, opts?: { dirty?: boolean }) => {
  ...
  const switchVarType = useCallback((id: string, nextType: "number" | "boolean") => {
    setDraftVars((prev) =>
      prev.map((variable) => {
  ...
  const toggleVarOpen = useCallback((id: string) => {
  ...
  const removeVarRow = useCallback((id: string) => {
  ...
  const saveVarRow = useCallback((row: VarRow): SaveVarResult => {
  ...
  const validateVarRows = useCallback((): ValidateVarRowsResult => {
    const variables: VarDef[] = [];
    const errorsById: Record<string, VarRowErrors> = {};

    for (const row of draftVars) {
      const result = rowToVarDefValidatedDetailed(row, draftVars);

      if (!result.ok) {
        errorsById[row.id] = result.errors;
        continue;
      }
    ...
    if (Object.keys(errorsById).length > 0) return { ok: false, errorsById };

    return { ok: true, variables };
  }, [draftVars]);

  const persistValidatedVars = useCallback((variables: VarDef[]) => {
    setDraftVars(variables.map(varDefToRow));
    setDirtyVarIds(new Set());
    for (const variable of variables) {
      const existedBefore = initialVars.some((persistedVar) => persistedVar.id === variable.id);
      onPersistSave?.(variable, existedBefore);
    }
  }, [initialVars, onPersistSave],
  );


  return {
    draftVars, openVarId, varNameRefs, dirtyVarIds, setDraftVars, setOpenVarId, isDirtyVar, markDirty, clearDirty, syncFromVars, computeRowErrors,
    updateVarRow, switchVarType, addVarRow, toggleVarOpen, removeVarRow, saveVarRow, validateVarRows, persistValidatedVars
  };
}